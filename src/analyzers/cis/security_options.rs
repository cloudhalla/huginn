...
impl Analyzer for UacAnalyzer {
    ...
    fn analyze(&self, info: &SystemInfo) -> Result<Vec<Finding>, HuginnError> {
        ...
        match sec.uac_enabled {
            None => findings.push(Finding::skip(
                "CIS-2.3.7.1",
                "User Account Control (UAC) — data unavailable",
                "UAC is a Windows-specific feature; data could not be collected on this platform.",
        ...
        match sec.secure_desktop {
            None => findings.push(Finding::skip(
                "CIS-2.3.7.2",
                "UAC secure desktop — data unavailable",
                "UAC is a Windows-specific feature; data could not be collected on this platform.",
                Category::SystemIntegrity,
            )),
            Some(false) => findings.push(
            ...
            Some(true) => findings.push(Finding::pass(
                "CIS-2.3.7.2",
                "UAC elevation prompt uses secure desktop",
                Category::SystemIntegrity,
            )),
        }
        ...
        match sec.uac_level {
            None => findings.push(Finding::skip(
                "CIS-2.3.7.3",
                "UAC notification level — data unavailable",
                "UAC is a Windows-specific feature; data could not be collected on this platform.",
                Category::SystemIntegrity,
            )),
            ...
            Some(_) => findings.push(Finding::pass(
                "CIS-2.3.7.3",
                "UAC notification level is appropriately configured",
                Category::SystemIntegrity,
            )),
        }
...
impl Analyzer for LsaProtectionAnalyzer {
    ...
    fn analyze(&self, info: &SystemInfo) -> Result<Vec<Finding>, HuginnError> {
        ...
        // LSA Protection (PPL)
        match sec.lsa_protection {
            None => findings.push(Finding::skip(
                "CIS-8.1.1",
                "LSA Protection (RunAsPPL) — data unavailable",
                "LSA protection is a Windows-specific feature; data could not be collected on this platform.",
                Category::SystemIntegrity,
            )),
            Some(false) => findings.push(
                Finding::fail(
        ...
        // Credential Guard
        match sec.credential_guard {
            None => findings.push(Finding::skip(
                "CIS-8.1.2",
                "Windows Credential Guard — data unavailable",
                "Credential Guard is a Windows-specific feature; data could not be collected on this platform.",
                Category::SystemIntegrity,
            )),
            Some(false) => findings.push(
                Finding::fail(
            ...
            Some(true) => findings.push(Finding::pass(
                "CIS-8.1.2",
                "Windows Credential Guard is enabled",
                Category::SystemIntegrity,
            )),
        }
        ...
        match sec.defender_enabled {
            None => findings.push(Finding::skip(
                "CIS-5.1",
                "Windows Defender — data unavailable",
                "Windows Defender is a Windows-specific feature; data could not be collected on this platform.",
            ...
            Some(true) => {
                if sec.defender_real_time == Some(false) {
                    findings.push(
                        Finding::fail(
                            "CIS-5.2",
                    ...
                    findings.push(Finding::pass(
                        "CIS-5.1",
                        "Windows Defender is enabled with real-time protection",
                        Category::SoftwareSecurity,
                    ));
                }
            }
